# hxluajit

![](https://img.shields.io/github/repo-size/ShadowEngineTeam/hxluajit) ![](https://badgen.net/github/open-issues/ShadowEngineTeam/hxluajit) ![](https://badgen.net/badge/license/MIT/green)

Haxe/hxcpp @:native bindings for [LuaJIT](https://luajit.org/luajit.html).

### Installation

You can install it through `Git`
```bash
haxelib git hxluajit https://github.com/ShadowEngineTeam/hxluajit.git
```

### Usage Example

Check out the [Examples Folder](examples/) for examples on how to use this library.

### Licensing

This **hxluajit fork** is licensed under the **MIT License**. See [LICENSE](LICENSE) for details.

**hxluajit** is made available under the **MIT License**. Check [hxluajit's Licence](./LICENSE-hxluajit) for more information.

![](https://avatars.githubusercontent.com/u/11773774?s=200&v=4)

**LuaJIT** is made available under the **MIT License**. Check [LuaJIT's Licence](https://github.com/LuaJIT/LuaJIT/blob/v2.1/COPYRIGHT) for more information.
